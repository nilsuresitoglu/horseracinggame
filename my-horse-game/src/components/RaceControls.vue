<template>
  <div class="race-controls">
    ...
    <div v-if="hasRaces" class="race-progress mt-3">
      <div class="progress">
        <div
          class="progress-bar"
          role="progressbar"
          :style="{
            width: `${(currentRaceIndex / raceSchedule.length) * 100}%`,
          }"
          :aria-valuenow="currentRaceIndex"
          :aria-valuemin="0"
          :aria-valuemax="raceSchedule.length"
        ></div>
      </div>
      <small class="text-muted">
        Race {{ Math.min(currentRaceIndex + 1, raceSchedule.length) }} of
        {{ raceSchedule.length }}
      </small>
    </div>
  </div>
