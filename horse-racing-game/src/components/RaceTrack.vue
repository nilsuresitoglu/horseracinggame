...
const displayRaceNumber = computed(() => {
  return currentRaceIndex.value + 1 > 6 ? 6 : currentRaceIndex.value + 1;
});

const updateRaceProgress = () => {
  if (!raceSchedule.value[currentRaceIndex.value]) return;
  ...
  raceInterval.value = setInterval(() => {
    currentRaceParticipants.value = currentRaceParticipants.value.map(
      (horse) => {
        ...
        const speed = horse.condition / 10 + Math.random() * 2;
