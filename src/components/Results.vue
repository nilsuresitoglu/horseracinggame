      ...
      >
        ...
        <div class="table-responsive">
          <table class="table table-striped">
            ...
            <tbody>
              <tr
                v-for="(horse, horseIndex) in sortedResults(result.results)"
                :key="horse.id"
              >
                <td>{{ horseIndex + 1 }}</td>
                ...
                <td>
                  {{
                    horse.performance !== undefined
                      ? horse.performance.toFixed(2)
                      : "-"
                  }}
                </td>
              </tr>
...
function sortedResults(results) {
  if (!results) return [];
  if (results.length && results[0].performance !== undefined) {
    return [...results].sort((a, b) => b.performance - a.performance);
  }
  // fallback: sort by progress if performance is missing
  if (results.length && results[0].progress !== undefined) {
